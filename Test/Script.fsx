...
let testList = [0. .. 9.]
let test2 = testList |> DrawingDebugUtils.FromList ((fun i -> [circle 10. |> at (10. * cos (Pi * float i / 5.), 10. * sin (Pi * float i / 5.)) |> withFill Brushes.Red]), ListDrawerOptions(bordersEnabled = true, bordersPen = { Pens.Black with Thickness = 0.1 }, gridCellSizes = Unconstrained))
...
let test4 = chessBoard |> DrawingDebugUtils.FromArray2D((fun b -> [square 100. |> at origin |> withFill b]), ListDrawerOptions(bordersEnabled = false))
...
let textTest =
    let getShapes = function
        ...
        | i, 0 ->
            [
                sprintf "%d" i |> at origin |> writtenWith Brushes.Blue
            ]
        | i, j when i = j ->
            [
                sprintf "%d" (i*j) |> at origin |> writtenWith Brushes.Red
            ]
        | i, j ->
            [
                sprintf "%d" (i*j) |> at origin |> writtenWith Brushes.Black
            ]
